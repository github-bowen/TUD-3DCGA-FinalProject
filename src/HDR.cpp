...
HDR::~HDR() {
    ...
    if (brdfTex)    glDeleteTextures(1, &brdfTex);
...
void HDR::cubeToPre(Shader& shader) {
    if (prefilterMap) {
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
    glUniform1i(shader.getUniformLocation("environmentMap"), 14);
    glUniformMatrix4fv(shader.getUniformLocation("projection"), 1, GL_FALSE, glm::value_ptr(captureProjection));
    ...
    for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
    {
        ...
        glUniform1f(shader.getUniformLocation("roughness"), roughness);
        for (unsigned int i = 0; i < 6; ++i)
        {
            glUniformMatrix4fv(shader.getUniformLocation("view"), 1, GL_FALSE, glm::value_ptr(captureViews[i]));
...
void HDR::brdfLUT(Shader& shader) {
    if (brdfTex) {
        glDeleteTextures(1, &brdfTex);
        brdfTex = 0;
    }
